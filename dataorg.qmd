...
This structure scales to a project. It does not scale to a team where six analysts each keep their own `data/raw/` extract of the same source, pulled on six different dates. That is the point at which shared, curated datasets start to beat per-project copies, and @sec-infrastructure covers what to move to and what it costs.
...
Plain text formats (CSV, TSV, JSON) are preferred for long-term storage and reproducibility. Proprietary binary formats (`.xlsx`, `.sav`, `.sas7bdat`) may become unreadable as software evolves. If you inherit data in these formats, @sec-legacy-data covers reading them into R and converting them to open formats.
