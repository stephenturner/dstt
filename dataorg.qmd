...
Broman and Woo's ["Data Organization in Spreadsheets"](https://www.tandfonline.com/doi/full/10.1080/00031305.2017.1375989) [@broman2018] offers twelve practical recommendations for anyone who stores data in Excel or similar tools. The core philosophy: structure your data so that it can be read directly into R or Python without manual cleanup.
...
::: {.callout-warning}
Excel silently converts many text strings to dates (e.g., `SEPT1` becomes a date). Consider storing year, month, and day in separate columns if you're collecting data in Excel, or prefix dates with an apostrophe to force text storage.
:::
...
Jenny Bryan's ["How to Name Files"](https://github.com/jennybc/how-to-name-files) [@bryan2015] lays out three principles that make file names work well for both humans and computers.
...
A consistent folder structure makes projects navigable by collaborators (and your future self). One widely used convention:

```
project/
├── data/
│   ├── raw/          # Original data, never modified
│   └── processed/    # Cleaned/derived data from scripts
├── code/             # Analysis scripts (R, Python, etc.)
├── output/
│   ├── figures/
│   └── tables/
├── docs/             # Reports, protocols, manuscripts
└── README.md         # Project overview and instructions
```

Key rules:

- **Raw data is sacred.** The `data/raw/` folder is read-only. Never overwrite or edit raw data files.
...
The [ProjectTemplate](http://projecttemplate.net/) (R) and [Cookiecutter Data Science](https://drivendata.github.io/cookiecutter-data-science/) (Python) tools can scaffold these structures automatically.
...
| `.rds` / `.pkl` | R or Python objects (with caveats) | Cross-language sharing |
...
**Use version control for code, not data.** Git is designed for text files. Large or binary data files should live in dedicated storage (cloud drives, data repositories, databases) and be referenced in your code, not committed to the repository.
