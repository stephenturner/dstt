...
`tidyverse` is a meta-package: installing it brings in a broad dependency tree, and `library(tidyverse)` attaches its core packages. @sec-packageloading-minimal presents the search-path argument for naming packages individually; the dependency argument points the same direction.
