...
A dictionary is more useful when the pipeline reads it rather than merely describing what the pipeline produces. Keeping metric definitions in a machine-readable file that both the R code and the dashboard consume is covered in @sec-bi-metrics.
...
Everything in this chapter documents specific code and specific datasets. The layer above it, documenting how the team itself works (setup, access requests, conventions, and the step-by-step protocol for each recurring product), is the team handbook covered in @sec-onboarding.
