...
Suppression rules also constrain design decisions upstream of the output. A dashboard permitting users to filter to a single county, a single month, and a single age group will eventually produce a cell below the threshold, whatever the default view displays. Publishing at quarterly rather than monthly granularity is a common remedy. Choosing the aggregation grain with these thresholds in mind, and applying suppression in code on the way out, is covered in @sec-bi-aggregate.
