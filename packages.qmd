...
## How R's search path works {#sec-packages-searchpath}
...
Load a package with `library()` only if you call its functions throughout the script using bare names. If a package appears once or twice, use `::` instead (see @sec-packages-doublecolon).
...
## Use `::` for one-off functions {#sec-packages-doublecolon}
...
## The `conflicted` package {#sec-packages-conflicted}
...
## Team implications {#sec-packages-team}
