...
Two requests deserve special preparation because they carry a cost and not just a permission. Standing up or joining a database platform is a budget decision as much as a technical one, and going in with sizing numbers rather than a product name changes the conversation (@sec-infrastructure). Getting a tool approved for AI use is gated by whatever policy your agency has or has not written (@sec-aigovernance).

Where team composition is within your control, name an IT representative as an actual member of the team rather than treating IT as an external ticket queue. Teams with an embedded representative clear resource and access barriers substantially faster (@sec-onboarding-it).
...
A written report, an interactive dashboard, a two-page brief, and a slide deck are all appropriate in different contexts. The analysis determines what can be said; the output format determines whether it is heard. See @sec-dashboards for guidance on building dashboards for data that needs to be explored or updated regularly. See @sec-comms for a more detailed treatment of message structure, visualization choices, and communicating uncertainty.
