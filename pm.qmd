...
The most important work in a data science project often happens before any data is touched. Peng and Matsui's *The Art of Data Science* [@peng2015art] frames the entire analysis process around a specific task: state the question precisely. Vague questions produce vague analyses. "Can you look at overdose trends?" is not a question that will lead anywhere useful. "Has the age distribution of overdose deaths in our state shifted since 2019, and does the pattern differ by rural versus urban counties?" is a question that can be answered.
...
- What would a satisfying answer look like: a number, a chart, a report, or a recommendation?
- What does success look like, and how will we know when we've reached it?

Use the answers to agree on the project's scope and deliverables.
...
**Plan time for delivery.** The gap between "the analysis is done" and "the findings are in front of the people who need them" is consistently larger than anticipated. Time for writing, review, revision, and presentation needs to be included in project plans.
...
Agree on who owns each deliverable, who reviews it, and who covers an absence. Use a shared repository and a review workflow (@sec-git, @sec-peerreview-mechanics). Record dependencies and decisions where the team can find them.

Include reproducibility in the acceptance criteria: a colleague should be able to rerun the analysis from the recorded inputs and environment. Allocate time for that check before delivery. @sec-environments covers restoring the environment; @sec-documentation covers the project instructions; @sec-onboarding-protocols covers recurring-product handoffs.

## Working with Your IT Team {#sec-pm-it}

IT is a necessary partner in institutional public health work. Server access, database credentials, software installation, network permissions, and data transfer approvals all flow through IT. Projects that treat IT as an afterthought routinely stall at the moment they are otherwise ready to move.
...
Two requests deserve special preparation because they carry a cost and not just a permission. Standing up or joining a database platform is a budget decision as much as a technical one, and a request should include measured storage and compute needs (@sec-infrastructure). Approval for an AI tool depends on agency policy (@sec-aigovernance).

Where team composition is within your control, include an IT representative in project planning to identify resource and access requirements early (@sec-onboarding-it).
...
These constraints shape what tools and workflows are actually feasible. It is better to understand them at project start than to build an analysis pipeline that cannot be used where the data actually lives. See @sec-governance for a more detailed treatment of data use agreements, HIPAA, disclosure review, and working in secure data environments.

## Communicating Findings {#sec-pm-communication}

Findings need to reach the people who can use them. Public health data science ultimately serves program and policy decisions made by people who did not run the analysis and may not read a methods section.
...
A written report, an interactive dashboard, a two-page brief, and a slide deck are all appropriate in different contexts. Choose a format that helps the intended audience use the findings. See @sec-dashboards for guidance on building dashboards for data that needs to be explored or updated regularly. See @sec-comms for a more detailed treatment of message structure, visualization choices, and communicating uncertainty.
