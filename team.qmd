...
The binding constraint on upskilling is almost never motivation or materials. Both are abundant: the resources in @sec-resources are free and excellent, and people generally want to grow. The constraint is protected time, and after that, the procurement rules that govern how professional development money can actually be spent (@sec-trainingbudget). Learning to work reproducibly while also delivering the weekly reports is nearly impossible if the weekly reports consume every hour. Capacity-building that is not given real, defended time on the calendar does not happen, no matter how good the intentions.
...
The artifact that consolidates those scattered practices into a checklist is a team handbook: one document with a single entry point, covering setup, access requests, conventions, update protocols for each recurring product, and who to contact about what. @sec-onboarding covers its contents, how to test it against someone who has never performed the setup, and how to keep it current.
